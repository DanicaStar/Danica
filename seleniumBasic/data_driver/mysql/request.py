...
class RequestInterface():
    ...
    def __new_param(self,param):
        try:
            if isinstance(param,str) and param.startswith('{'):
                new_param=eval(param)   #如果接口请求参数是一个字符串类型的字典，则用eval函数将其还原成字典形式
    ...
    def __http_post(self,interface_url,headerdata,interface_param):
        #__http_post是一个私有方法，只能在该类的内部调用，调用语法self.__http_post
        '''
        :param interface_url: 接口地址
        :param headerdata: 请求他文件
        ...
        try:
            if interface_url !='':
                ...
                #verify参数是Boolen类型，默认为True，即启动HTTPS的SSL证书验证，此处不需要
                ...
                if response.status_code==200:
                    ...
                    result={'code':'2004','message':'接口参数为空','data':[]}
            elif interface_url=='':
...
if __name__ == '__main__':
    ...
    if params_interface['code']=='0000':
        ...
        url_interface=data['url_interface']
        temp=data['header_interface']
        headerdata=eval(data['header_interface'])    #将unicode转换成字典
        param_interface=data['params_interface']
        type_interface=data['exe_mode']
        if url_interface != '' and headerdata != '' and param_interface !='' and type_interface != '':
            ...
            if result['code']=='0000':
                ...
                test_db.op_sql('update case_interface set result_interface=%s"%s" where id=1'%result_resp)  #将结果更新到case_interface表中
